...
describe('Footer component', () => {
  ...
  it('renders the title text correctly', () => {
    renderFooter();
    expect(screen.getByText('Climate Transition Scenarios Repository')).toBeInTheDocument();
  });
  ...
  it('contains Contact Us email link', () => {
    ...
    expect(contactLink.closest('a')).toHaveAttribute('href', 'mailto:contact@rmi.org');
  });

  it('displays the current year in the copyright notice', () => {
    renderFooter();
    const currentYear = new Date().getFullYear().toString();
    expect(screen.getByText(new RegExp(`© ${currentYear} RMI`))).toBeInTheDocument();
  });

  it('contains the BarChart3 icon', () => {
    renderFooter();
    // Since icons don't have accessible text, we check for their parent container
    const titleSection = screen.getByText('Climate Transition Scenarios Repository').closest('div');
